...
데이터 출처 (공식, 무인증, 공개):
  - 국가 목록: https://www.gov.uk/api/search.json?filter_format=travel_advice
...
def fetch_country_list():
    """GOV.UK 검색 API로 foreign-travel-advice 문서(=국가) 전체 목록을 가져온다."""
    countries = []
    start = 0
    page_size = 100
    while True:
        params = {
            ...
            "count": page_size,
            "start": start,
            "fields": "title,base_path,public_timestamp",
        }
        resp = requests.get(
            SEARCH_API, params=params, headers={"User-Agent": USER_AGENT}, timeout=30
        )
...
def main():
    print("[1/3] FCDO 여행경보 국가 목록 가져오는 중...", file=sys.stderr)
